...
toy_supervised_dilution_test <- function(n = 301) {
  ...
  dcv_res <- DiCoVarML::tune.dicovar(X = as.data.frame(toy_data[, -1]),
                                     ...
                                     ensemble = c("ranger", "glmnet", "rangerE"),
                                     test.parts = 3:5)
...
#'
#' @importFrom magrittr %>%
iris_supervised_DCV_test <- function(X = datasets::iris[-5],
                                       Y = datasets::iris$Species,
                                       mean_dilution = NA,
                                       sd_dilution = NA,
                                       add_junk = c(),
  ...
  # optionally permute the Y labels to generate null results
  if(permute) {
    Y <- sample(Y)
  }
  ...
  # randomly select 20% of data as validation set
  dv_folds <- caret::createDataPartition(y = Y, times = 1,
                                         p = 0.8, list = FALSE)
  dv_fold_ids <- purrr::rep_along(Y, 2)
  # 1-is train data; 2 is test data
  dv_fold_ids[dv_folds] <- 1
  ...
  # udcv_res <- unsupervised.dicovar(X = as.data.frame(toy_data[, -1]),
  #                                  alpha = 0.5)
  dcv_res <- DiCoVarML::tune.dicovar(X = X,
                                     Y = Y,
                                     sample_ids = seq_along(Y),
                                     dv_labels = dv_fold_ids,
                                     ensemble = c("ranger", "glmnet", "rangerE"),
                                     test.parts = 3:4)
